...
function getStatusClass(status) {
    switch (status) {
        case 'Ontime': return 'status-ontime';// GLOBAL VARIABLES
let map = null;
let busMarker = null;
let destinationMarker = null; 
let allRoutes = []; 
let allStops = []; 
let passengerLocation = null;
// trackingListener now holds the onSnapshot function to UNSUBSCRIBE from the trip list
let trackingListener = null; 
let mapInitialized = false; 
let tempPassengerMarker = null;
let passengerWatchId = null; 
 ...
 * This is where the fix for real-time schedule updates is applied.
 */
async function searchBuses() {
    ...
    try {
        ...
        // This listener will fire every time a trip is created, updated, or deleted.
...
function showDetails(tripId, vehicleName) {
    ...
    trackingListener = db.collection('trips').doc(tripId).onSnapshot(async doc => {
        ...
        // --- CRITICAL: UI elements MUST be retrieved inside the listener ---
        ...
        if (trip.last_known_location) {
            ...
            // Notification Logic (Enhanced notification text)
...
function hideDetails() {
    ...
    // Rerun the search to re-establish the schedule list listener if the user wants to continue tracking other buses
    const routeId = document.getElementById('routeSelect').value;
    if (routeId) {
        searchBuses();
    }
